...
namespace Razomy.Unity.Scripts.Common.Unity
{
  ...
  {
    private static readonly string[,] readNames2D =
    {
...
static string[,] readNames3D = new string[,]
{
  ...
  {"read3DC4", "_Tex3D", "_Buffer3DC4"}
    ...
    */
    ...
    public static void ReadFromRenderTexture(RenderTexture tex, int channels, ComputeBuffer buffer,
      ComputeShader readData)
    {
      if (tex == null)
      {
      ...
      readData.Dispatch(kernel, Mathf.Max(1, width / 8 + padX), Mathf.Max(1, height / 8 + padY),
        Mathf.Max(1, depth / 8 + padZ));
    }

    public static void ReadSingleFromRenderTexture(RenderTexture tex, float x, float y, float z, ComputeBuffer buffer,
      ComputeShader readData, bool useBilinear)
    {
      if (tex == null)
      {
