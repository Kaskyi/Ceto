...
namespace Razomy.Unity.Scripts.UnderWater
{
  ...
  public class UnderWater : OceanComponent
  {
    ...
    /// </summary>
    [Range(0.0f, 1.0f)] public float edgeFade = 0.2f;
    ...
    /// </summary>
    [Range(0.0f, 1.0f)] public float absorptionR = 0.45f;
    ...
    ///   as the light travels through the water.
    /// </summary>
    [Range(0.0f, 1.0f)] public float absorptionG = 0.029f;
    ...
    ///   as the light travels through the water.
    /// </summary>
    [Range(0.0f, 1.0f)] public float absorptionB = 0.018f;

    /// <summary>
    ///   Modify the result of applying the absorption cof when above the mesh looking down.
    /// </summary>
    public AbsorptionModifier aboveAbsorptionModifier = new(2.0f, 1.0f, Color.white);

    /// <summary>
    ///   Modify the result of applying the absorption cof when below the mesh looking down.
    /// </summary>
    public AbsorptionModifier belowAbsorptionModifier = new(0.1f, 1.0f, Color.white);

    /// <summary>
    ///   Modify the result of applying the absorption cof to the subsurface scatter.
    /// </summary>
    public AbsorptionModifier subSurfaceScatterModifier = new(10.0f, 1.5f, new Color32(220, 250, 180, 255));

    /// <summary>
    ///   Modify the inscatter when above the mesh looking down.
    /// </summary>
    public InscatterModifier aboveInscatterModifier = new(50.0f, 1.0f, new Color32(2, 25, 43, 255), INSCATTER_MODE.EXP);

    /// <summary>
    ///   Modify the inscatter when below the mesh looking up.
    /// </summary>
    public InscatterModifier belowInscatterModifier = new(60.0f, 1.0f, new Color32(7, 51, 77, 255), INSCATTER_MODE.EXP);

    /// <summary>
    ///   Modifies how the caustics are applied.
    /// </summary>
    public CausticModifier causticModifier = new(0.25f, 0.25f, 0.1f, 0.75f, Color.white);
    ...
    /// <summary>
    /// </summary>
    [HideInInspector] public Shader oceanBottomSdr, oceanDepthSdr, copyDepthSdr;

    [HideInInspector] public Shader oceanMaskSdr, oceanMaskFlippedSdr, normalFadeSdr;
    ...
    [Range(0.0f, 1.0f)]
    /*public*/
    private readonly float depthBlend = 0.2f;

    /// <summary>
    ///   Used as the value the ocean depths are normalized to.
    ///   Dont change this.
    /// </summary>
    private readonly float MAX_DEPTH_DIST = 500.0f;
    ...
    private void Start()
    {
      try
      {
        var mesh = CreateBottomMesh(32, 512);

        //The bottom used to render the masks.
        m_bottomMask = new GameObject("Ceto Bottom Mask Gameobject");

        var filter = m_bottomMask.AddComponent<MeshFilter>();
        var renderer = m_bottomMask.AddComponent<MeshRenderer>();
        var willRender = m_bottomMask.AddComponent<NotifyOnWillRender>();
    ...
    private void Update()
    {
      try
      {
        ...
        var absCof = new Vector4(absorptionR, absorptionG, absorptionB, 1.0f);
        var sssCof = absCof;
        var belowCof = absCof;
        ...
        var absTint = aboveAbsorptionModifier.tint * Mathf.Max(0.0f, aboveAbsorptionModifier.intensity);
        var sssTint = subSurfaceScatterModifier.tint * Mathf.Max(0.0f, subSurfaceScatterModifier.intensity);
        var belowTint = belowAbsorptionModifier.tint * Mathf.Max(0.0f, belowAbsorptionModifier.intensity);

        var causticParam = new Vector4();
        ...
        var causticDistortion = new Vector2();
        ...
        var aboveInscatterCol = aboveInscatterModifier.color;
        ...
        var belowInscatterCol = belowInscatterModifier.color;
        ...
        Shader.SetGlobalTexture("Ceto_CausticTexture",
          causticTexture.tex != null ? causticTexture.tex : Texture2D.blackTexture);
    ...
    protected override void OnDestroy()
    {
      ...
      try
      {
        ...
        {
          var mesh = m_bottomMask.GetComponent<MeshFilter>().mesh;
    ...
    private void FitBottomToCamera()
    {
      ...
      var far = cam.farClipPlane * 0.85f;
    ...
    private void UpdateBottomBounds()
    {
      ...
      {
        var bounds = new Bounds(new Vector3(0.0f, level, 0.0f), new Vector3(bigNumber, bottomDepth, bigNumber));
    ...
    public void RenderOceanMask(GameObject go)
    {
      try
      {
        ...
        if (cam.name == "SceneCamera" || cam.GetComponent<UnderWaterPostEffect>() == null ||
            SystemInfo.graphicsShaderLevel < 30 || GetDisableUnderwater(data.settings))
        {
        ...
        else
        {
          ...
          var sdr = m_ocean.Projection.IsFlipped ? oceanMaskFlippedSdr : oceanMaskSdr;
    ...
    /// </summary>
    private void RenderSteroOceanMask(MaskData data, Camera cam, Shader sdr)
    {
    ...
    /// </summary>
    private void RenderOceanMask(MaskData data, RenderTexture target, Vector3 position, Quaternion rotation,
      Matrix4x4 projection, Shader sdr)
    {
    ...
    */
    ...
    ///   Used to convert the depth to world position
    /// </summary>
    private void BindIVPMatrix(Matrix4x4 projection, Matrix4x4 view)
    {
      var ivp = projection * view;
      Shader.SetGlobalMatrix("Ceto_Camera_IVP0", ivp.inverse);
    }
    ...
    /// </summary>
    private void RenderSteroOceanDepth(DepthData data, Camera cam)
    {
#if UNITY_540_OR_HIGHER && CETO_USE_STEAM_VR
            if (OceanVR.OpenVRInUse)
            {
    ...
    ///   at the provided camera position.
    /// </summary>
    private void RenderOceanDepth(DepthData data, RenderTexture target, Vector3 position, Quaternion rotation,
    ...
    private void CreateMaskCameraFor(Camera cam, MaskData data)
    {
      if (data.cam == null)
      {
        var go = new GameObject("Ceto Mask Camera: " + cam.name);
    ...
    private void CreateDepthCameraFor(Camera cam, DepthData data)
    {
      if (data.cam == null)
      {
        var go = new GameObject("Ceto Depth Camera: " + cam.name);
        go.hideFlags = HideFlags.HideAndDontSave;
        go.AddComponent<IgnoreOceanEvents>();
        go.AddComponent<DisableFog>();
        go.AddComponent<DisableShadows>();
      ...
      if (data.target0 == null || data.target0.width != cam.pixelWidth || data.target0.height != cam.pixelHeight)
      {
        ...
        var format = RenderTextureFormat.RGFloat;
    ...
    {
      var vertices = new Vector3[segementsX * segementsY];
      var texcoords = new Vector2[segementsX * segementsY];

      var TAU = Mathf.PI * 2.0f;
      float r;
      for (var x = 0; x < segementsX; x++)
      for (var y = 0; y < segementsY; y++)
      {
        r = x / (float)(segementsX - 1);

        vertices[x + y * segementsX].x = r * Mathf.Cos(TAU * y / (segementsY - 1));
        vertices[x + y * segementsX].y = 0.0f;
        vertices[x + y * segementsX].z = r * Mathf.Sin(TAU * y / (segementsY - 1));

        if (x == segementsX - 1) vertices[x + y * segementsX].y = 1.0f;
      }

      var indices = new int[segementsX * segementsY * 6];

      var num = 0;
      for (var x = 0; x < segementsX - 1; x++)
      for (var y = 0; y < segementsY - 1; y++)
      {
