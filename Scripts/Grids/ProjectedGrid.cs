...
namespace Razomy.Unity.Scripts.Grids
{
  ...
  public class ProjectedGrid : OceanComponent
  {
    ...
    /// </summary>
    private readonly float borderLength = 200.0f;
    ...
    /// </summary>
    /*public*/
    private readonly GRID_GROUPS gridGroups = GRID_GROUPS.SINGLE;

    /// <summary>
    ///   The mesh reflection probes usage.
    ///   Currently disabled as untested.
    /// </summary>
    private readonly ReflectionProbeUsage reflectionProbes = ReflectionProbeUsage.Off;
    ...
    /// </summary>
    private Dictionary<int, Grid> m_grids = new();
    ...
    private void Update()
    {
      try
      {
        ...
        var r = ResolutionToNumber(resolution);
        var screenGridSize = new Vector2(r / (float)ScreenWidth(), r / (float)ScreenHeight());
        ...
        while (e.MoveNext())
        {
          var grid = e.Current.Value;
          //If this mesh is the one for the current resolution setting.
          var active = e.Current.Key == (int)resolution;
        ...
        if (!UnderSideNeeded())
        {
          ...
          while (e.MoveNext()) Activate(e.Current.Value.under, false);
        }
        ...
        {
          e = m_grids.GetEnumerator();
          while (e.MoveNext()) Activate(e.Current.Value.top, false);
        }
    ...
    private void UpdateGrid(Grid grid)
    {
      ...
      var count = grid.topRenderer.Count;
      for (var i = 0; i < count; i++)
      {
    ...
    /// </summary>
    private void ResetBounds(Grid grid)
    {
      var level = m_ocean.level;
      var range = m_ocean.FindMaxDisplacement(true);
      var bigNumber = 1e8f;
      var count = 0;

      var bounds = new Bounds(Vector3.zero, new Vector3(bigNumber, level + range, bigNumber));

      count = grid.topFilters.Count;
      for (var i = 0; i < count; i++) grid.topFilters[i].mesh.bounds = bounds;

      count = grid.underFilters.Count;
      for (var i = 0; i < count; i++) grid.underFilters[i].mesh.bounds = bounds;
    }
    ...
    private void CreateGrid(MESH_RESOLUTION meshRes)
    {
      ...
      var resolution = ResolutionToNumber(meshRes);
      var groups = ChooseGroupSize(resolution, gridGroups, screenWidth, screenHeight);
      ...
      var meshs = CreateScreenQuads(resolution, groups, screenWidth, screenHeight);
      ...
      foreach (var mesh in meshs)
      {
        if (oceanTopSideMat != null)
        {
          var top = new GameObject("Ceto TopSide Grid LOD: " + meshRes);

          var filter = top.AddComponent<MeshFilter>();
          var renderer = top.AddComponent<MeshRenderer>();

          //The notify script will call the added functions when OnWillRender is called. 
          var willRender = top.AddComponent<NotifyOnWillRender>();
        ...
        {
          var under = new GameObject("Ceto UnderSide Grid LOD: " + meshRes);

          var filter = under.AddComponent<MeshFilter>();
          var renderer = under.AddComponent<MeshRenderer>();

          //The notify script will call the added functions when OnWillRender is called.
          var willRender = under.AddComponent<NotifyOnWillRender>();
    ...
    private void ClearGrid(Grid grid)
    {
      ...
      {
        var count = grid.topFilters.Count;
        for (var i = 0; i < count; i++)
        {
      ...
      {
        var count = grid.underFilters.Count;
        for (var i = 0; i < count; i++)
        {
    ...
    private int ChooseGroupSize(int resolution, GRID_GROUPS groups, int width, int height)
    {
      ...
      var groupSize = GroupToNumber(groups);
    ...
    private IList<Mesh> CreateScreenQuads(int resolution, int groupSize, int width, int height)
    {
      ...
      if (groupSize != -1)
      {
        ...
        w = groupSize / (float)width;
        h = groupSize / (float)height;
      }
      else
      {
        numVertsX = width / resolution;
      ...
      for (var y = 0; y < numY; y++)
      {
        ...
        var quad = CreateQuad(numVertsX, numVertsY, ux, uy, w, h);
        quads.Add(quad);
      }
    ...
    {
      var vertices = new Vector3[numVertsX * numVertsY];
      var texcoords = new Vector2[numVertsX * numVertsY];
      var indices = new int[numVertsX * numVertsY * 6];

      //Percentage of verts that will be in the border.
      //Only a small number is needed.
      var border = 0.1f;

      for (var x = 0; x < numVertsX; x++)
      for (var y = 0; y < numVertsY; y++)
      {
        Vector2 uv = new Vector3(x / (float)(numVertsX - 1), y / (float)(numVertsY - 1));
      ...
      var num = 0;
      for (var x = 0; x < numVertsX - 1; x++)
      for (var y = 0; y < numVertsY - 1; y++)
      {
