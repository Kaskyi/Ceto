...
namespace Razomy.Unity.Scripts.Ocean
{
  ...
  public class Projection3d : IProjection
  {
    ...
    private readonly double[] m_ab;
    private readonly double[] m_b;

    /// <summary>
    ///   The corner points of a frustum box.
    /// </summary>
    private readonly List<double[]> m_corners;

    private readonly double[] m_dir;

    /// <summary>
    ///   The frustum corners in world space.
    /// </summary>
    private readonly List<double[]> m_frustumCorners;

    private readonly double[] m_lookAt;
    ...
    private readonly double[] m_p0;
    private readonly double[] m_p1;

    /// <summary>
    /// </summary>
    private readonly List<double[]> m_pointList;

    private readonly double[] m_pos;

    /// <summary>
    ///   The projector range and interpolation matrix.
    /// </summary>
    private readonly double[] m_projectorI;

    /// <summary>
    ///   The projector projection, view and inverse view projection matrix.
    /// </summary>
    private readonly double[] m_projectorIVP;

    /// <summary>
    ///   The projector projection, view and inverse view projection matrix.
    /// </summary>
    private readonly double[] m_projectorP;

    /// <summary>
    ///   The projector range and interpolation matrix.
    /// </summary>
    private readonly double[] m_projectorR;

    /// <summary>
    ///   The projector projection, view and inverse view projection matrix.
    /// </summary>
    private readonly double[] m_projectorV;

    /// <summary>
    ///   The projector projection, view and inverse view projection matrix.
    /// </summary>
    private readonly double[] m_projectorVP;
    ...
    /// </summary>
    private readonly List<double[]> m_quad;

    private readonly double[] m_up;

    private readonly double[] m_xaxis;
    private readonly double[] m_yaxis;
    private readonly double[] m_zaxis;

    /// <summary>
    ///   Buffers used to reduce memory allocations
    /// </summary>
    private readonly double[] MATRIX_BUFFER0;

    /// <summary>
    ///   Buffers used to reduce memory allocations
    /// </summary>
    private readonly double[] MATRIX_BUFFER1;

    /// <summary>
    ///   Buffers used to reduce memory allocations
    /// </summary>
    private readonly double[] VECTOR_BUFFER;
    ...
    /// </summary>
    public void UpdateProjection(Camera camera, CameraData.CameraData data)
    {
      var cam = camera;
    ...
    private void AimProjector(Camera cam)
    {
      ...
      var fit = TightFit ? 20.0 : 50.0;

      var range = Math.Max(0.0, m_ocean.FindMaxDisplacement(true)) + fit;
    ...
    private void CreateRangeMatrix(Camera cam, double[] projectorVP)
    {
      ...
      double level = m_ocean.level;
      var range = Math.Max(1.0, m_ocean.FindMaxDisplacement(true));
      ...
      m_projectorR[0] = xmax - xmin;
      m_projectorR[12] = xmin;
      m_projectorR[5] = ymax - ymin;
      m_projectorR[13] = ymin;
    }
    ...
    private void HProject(double[] ivp, double[] corner, double[] result)
    {
      ...
      var t = (m_a[3] * h - m_a[1]) / (m_ab[1] - m_ab[3] * h);
    ...
    {
      var invLength = 1.0 / Math.Sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
