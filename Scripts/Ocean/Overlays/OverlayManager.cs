...
namespace Ceto
{
  ...
  {
    private static readonly Vector2 TEXTURE_FOAM = new(1, 0);
    private static readonly Vector2 DONT_TEXTURE_FOAM = new(0, 1);

    /// <summary>
    ///   A texture that when sampled from
    ///   as a normal will give 0.
    /// </summary>
    private readonly Texture2D m_blankNormal;

    private readonly Color m_clearColor;
    private readonly List<WaveOverlay> m_clipOverlays;

    /// <summary>
    ///   The m_containing that contain the query pos.
    /// </summary>
    private readonly List<QueryableOverlayResult> m_containingOverlays;

    private readonly List<WaveOverlay> m_foamOverlays;

    /// <summary>
    ///   Temp arrays to hold overlays when they
    ///   get separated out before rendering.
    /// </summary>
    private readonly List<WaveOverlay> m_heightOverlays;

    private readonly List<WaveOverlay> m_normalOverlays;

    /// <summary>
    ///   The overlay material.
    /// </summary>
    private readonly Material m_overlayMat;
    ...
    /// </summary>
    private readonly List<WaveOverlay> m_queryableOverlays;

    /// <summary>
    ///   Tmp list of overlays that need to be removed.
    /// </summary>
    private readonly List<WaveOverlay> m_removeOverlays;

    /// <summary>
    ///   The overlays.
    /// </summary>
    private readonly LinkedList<WaveOverlay> m_waveOverlays;
    ...
    public void QueryWaves(WaveQuery query)
    {
      ...
      var clipOnly = query.mode == QUERY_MODE.CLIP_TEST;
      ...
      for (var i = 0; i < m_containingOverlays.Count; i++)
      {
        var result = m_containingOverlays[i];
        ...
        try
        {
          ...
          {
            var clip = (clipTex.tex as Texture2D).GetPixelBilinear(result.u, result.v).a;
          ...
          {
            var alpha = heightTex.alpha;
            var maskAlpha = Mathf.Max(0.0f, heightTex.maskAlpha);
      ...
      if (0.5f - clipSum < 0.0f) query.result.isClipped = true;
    ...
    public void Release()
    {
      Object.DestroyImmediate(m_blankNormal);
    }
    ...
    public void Update()
    {
      ...
      for (var i = 0; i < m_removeOverlays.Count; i++) m_waveOverlays.Remove(m_removeOverlays[i]);
    }
    ...
    {
      for (var i = 0; i < m_queryableOverlays.Count; i++)
      {
        var overlay = m_queryableOverlays[i];

        if (overlay.Hide) continue;

        var b1 = (overrideIqnoreQuerys || !overlay.HeightTex.ignoreQuerys) && overlay.HeightTex.IsDrawable;
        var b2 = (overrideIqnoreQuerys || !overlay.ClipTex.ignoreQuerys) && overlay.ClipTex.IsDrawable;
    ...
    public void GetQueryableContaining(float x, float z, bool overrideIqnoreQuerys, bool clipOnly)
    {
      ...
      for (var i = 0; i < m_queryableOverlays.Count; i++)
      {
        ...
        var b1 = !clipOnly && (overrideIqnoreQuerys || !overlay.HeightTex.ignoreQuerys) && overlay.HeightTex.IsDrawable;
        var b2 = (overrideIqnoreQuerys || !overlay.ClipTex.ignoreQuerys) && overlay.ClipTex.IsDrawable;
    ...
    public void RenderWaveOverlays(Camera cam, WaveOverlayData data)
    {
      ...
      while (e.MoveNext())
      {
        var overlay = e.Current;

        //If camera farther than draw distance dont draw overlay.
        var distance = (overlay.Position - cam.transform.position).magnitude;
        ...
        {
          if (overlay.HeightTex.IsDrawable) m_heightOverlays.Add(overlay);

          if (overlay.NormalTex.IsDrawable) m_normalOverlays.Add(overlay);

          if (overlay.FoamTex.IsDrawable) m_foamOverlays.Add(overlay);

          if (overlay.ClipTex.IsDrawable) m_clipOverlays.Add(overlay);
        }
      }
    ...
    private void RenderHeightOverlays(RenderTexture target)
    {
      ...
      var pass = HeightOverlayBlendMode == OVERLAY_BLEND_MODE.ADD
        ? (int)OVERLAY_PASS.HEIGHT_ADD
        : (int)OVERLAY_PASS.HEIGHT_MAX;

      for (var i = 0; i < m_heightOverlays.Count; i++)
      {
        var overlay = m_heightOverlays[i];
        ...
        m_overlayMat.SetTexture("Ceto_Overlay_Height",
          overlay.HeightTex.tex != null ? overlay.HeightTex.tex : Texture2D.blackTexture);
        m_overlayMat.SetTexture("Ceto_Overlay_HeightMask",
          overlay.HeightTex.mask != null ? overlay.HeightTex.mask : Texture2D.blackTexture);
    ...
    private void RenderNormalOverlays(RenderTexture target)
    {
      ...
      for (var i = 0; i < m_normalOverlays.Count; i++)
      {
        var overlay = m_normalOverlays[i];
        ...
        m_overlayMat.SetTexture("Ceto_Overlay_Normal",
          overlay.NormalTex.tex != null ? overlay.NormalTex.tex : m_blankNormal);
        m_overlayMat.SetTexture("Ceto_Overlay_NormalMask",
          overlay.NormalTex.mask != null ? overlay.NormalTex.mask : Texture2D.blackTexture);
        m_overlayMat.SetFloat("Ceto_Overlay_MaskMode", (float)overlay.NormalTex.maskMode);

        Blit(overlay.Corners, overlay.NormalTex.scaleUV, overlay.NormalTex.offsetUV, target, m_overlayMat,
          (int)OVERLAY_PASS.NORMAL_ADD);
      }
    }

    private void RotateY(Matrix4x4 m, float angle)
    {
      var ca = Mathf.Cos(angle * Mathf.PI / 180.0f);
      var sa = Mathf.Sin(angle * Mathf.PI / 180.0f);
    ...
    private void RenderFoamOverlays(RenderTexture target)
    {
      ...
      var pass = FoamOverlayBlendMode == OVERLAY_BLEND_MODE.ADD
        ? (int)OVERLAY_PASS.FOAM_ADD
        : (int)OVERLAY_PASS.FOAM_MAX;

      for (var i = 0; i < m_foamOverlays.Count; i++)
      {
        var overlay = m_foamOverlays[i];
        ...
        m_overlayMat.SetTexture("Ceto_Overlay_Foam",
          overlay.FoamTex.tex != null ? overlay.FoamTex.tex : Texture2D.blackTexture);
        m_overlayMat.SetTexture("Ceto_Overlay_FoamMask",
          overlay.FoamTex.mask != null ? overlay.FoamTex.mask : Texture2D.blackTexture);
    ...
    private void RenderClipOverlays(RenderTexture target)
    {
      ...
      for (var i = 0; i < m_clipOverlays.Count; i++)
      {
        var overlay = m_clipOverlays[i];

        m_overlayMat.SetFloat("Ceto_Overlay_Alpha", Mathf.Max(0.0f, overlay.ClipTex.alpha));
        m_overlayMat.SetTexture("Ceto_Overlay_Clip",
          overlay.ClipTex.tex != null ? overlay.ClipTex.tex : Texture2D.blackTexture);

        Blit(overlay.Corners, overlay.ClipTex.scaleUV, overlay.ClipTex.offsetUV, target, m_overlayMat,
          (int)OVERLAY_PASS.CLIP_ADD);
      }
    }
    ...
    /// </summary>
    public void CreateOverlays(Camera cam, WaveOverlayData overlay, OVERLAY_MAP_SIZE normalOverlaySize,
      OVERLAY_MAP_SIZE heightOverlaySize, OVERLAY_MAP_SIZE foamOverlaySize, OVERLAY_MAP_SIZE clipOverlaySize)
    {
    ...
    /// </summary>
    public void CreateBuffer(string name, Camera cam, OVERLAY_MAP_SIZE size, RenderTextureFormat format, bool isLinear,
      ref RenderTexture map)
    {
      var S = SizeToValue(size);

      var w = Mathf.Min(4096, (int)(cam.pixelWidth * S));
      var h = Mathf.Min(4096, (int)(cam.pixelHeight * S));
      ...
      if (map == null || map.width != w || map.height != h)
      {
        ...
        var rw = isLinear ? RenderTextureReadWrite.Linear : RenderTextureReadWrite.Default;
