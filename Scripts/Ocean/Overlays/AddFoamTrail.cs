...
namespace Razomy.Unity.Scripts.Ocean.Overlays
{
  ...
  public class AddFoamTrail : AddWaveOverlayBase, IOverlayParentBounds
  {
    ...
    ///   The overlays alpha.
    /// </summary>
    [Range(0.0f, 2.0f)] public float alpha = 0.8f;

    /// <summary>
    ///   Randomizes the spin and expansion values
    ///   for each overlay created.
    /// </summary>
    [Range(0.0f, 1.0f)] public float jitter = 0.2f;
    ...
    /// </summary>
    private readonly LinkedList<FoamOverlay> m_pool = new();

    /// <summary>
    ///   Tmp buffer to hold removed overlays
    /// </summary>
    private readonly List<FoamOverlay> m_remove = new();
    ...
    /// </summary>
    private readonly float MAX_MOVEMENT = 100.0f;
    ...
    private void AddFoam()
    {
      ...
      var distance = m_lastPosition - pos;
      ...
      var momentumDir = direction * momentum;
      ...
      while (m_remainingDistance > spacing)
      {
        ...
        var overlayPos = pos + direction * d;

        //Create a new overlay and set is starting values.
        var overlay = NewFoamOverlay(overlayPos, Rotation(), size, duration, foamTexture);
    ...
    private void UpdateOverlays()
    {
      ...
      for (var i = 0; i < m_overlays.Count; i++)
      {
        ...
        var half = Mathf.Max(overlay.HalfSize.x, overlay.HalfSize.y);
    ...
    private void RemoveOverlays()
    {
      ...
      for (var i = 0; i < m_overlays.Count; i++)
      {
        var overlay = m_overlays[i] as FoamOverlay;
      ...
      for (var i = 0; i < m_remove.Count; i++)
      {
