...
namespace Ceto
{
  ...
  public class AddAutoShoreMask : AddWaveOverlayBase
  {
    ...
    /// </summary>
    [Range(0.1f, 100.0f)] public float heightSpread = 10.0f;

    /// <summary>
    ///   The higher the value the larger the mask radius around the terrain.
    /// </summary>
    [Range(0.1f, 10.0f)] public float foamSpread = 2.0f;

    /// <summary>
    ///   The higher the value the further inshore the clipped area will be
    /// </summary>
    [Range(0.1f, 10.0f)] public float clipOffset = 4.0f;
    ...
    [Range(0, 1)] public float heightAlpha = 0.9f;
    ...
    [Range(0, 1)] public float normalAlpha = 0.8f;
    ...
    [Range(0, 10)] public float edgeFoamAlpha = 1.0f;
    ...
    [Range(0, 1)] public float foamMaskAlpha = 1.0f;
    ...
    /// <summary>
    /// </summary>
    private Texture2D m_heightMask, m_edgeFoam, m_clipMask;

    /// <summary>
    /// </summary>
    private float m_heightSpread, m_foamSpread, m_clipOffset, m_resolution;
    ...
    {
      if (Ocean.Instance != null && (!m_registered || SettingsChanged())) CreateShoreMasks();

      var halfSize = new Vector2(m_width * 0.5f, m_height * 0.5f);
    ...
    private void CreateShoreMasks()
    {
      ...
      if (terrain == null)
      {
        ...
        Ocean.LogWarning(
          "The AddAutoShoreMask script must be attached to a component with a Terrain. The shore mask will not be created.");
        enabled = false;
        return;
      }
      ...
      var level = Ocean.Instance.level;

      var data = ShoreMaskGenerator.CreateHeightMap(terrain);

      var actualResolution = terrain.terrainData.heightmapResolution;
      var heightMap = new InterpolatedArray2f(data, actualResolution, actualResolution, 1, false);

      if (useHeightMask || useNormalMask || useFoamMask)
        m_heightMask = ShoreMaskGenerator.CreateMask(heightMap, resolution, resolution, level, heightSpread,
          TextureFormat.ARGB32);

      if (useEdgeFoam)
        m_edgeFoam =
          ShoreMaskGenerator.CreateMask(heightMap, resolution, resolution, level, foamSpread, TextureFormat.ARGB32);

      if (useClipMask)
        m_clipMask = ShoreMaskGenerator.CreateClipMask(heightMap, resolution, resolution, level + clipOffset,
          TextureFormat.ARGB32);
