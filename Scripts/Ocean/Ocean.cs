...
namespace Ceto
{
  ...
  public class Ocean : MonoBehaviour, ICoroutine
  {
    ...
    ///   The wind direction.
    /// </summary>
    [Range(0.0f, 360.0f)] public float windDir;

    /// <summary>
    ///   The roughness for the BRDF Lighting.
    /// </summary>
    [Range(0.0f, 1.0f)] public float specularRoughness = 0.2f;

    /// <summary>
    ///   The roughness for the BRDF Lighting.
    /// </summary>
    [Range(0.0f, 1.0f)] public float specularIntensity = 0.2f;

    /// <summary>
    ///   The power for the fresnel.
    /// </summary>
    [Range(0.0f, 10.0f)] public float fresnelPower = 5.0f;

    /// <summary>
    ///   The minimum fresnel.
    /// </summary>
    [Range(0.0f, 1.0f)] public float minFresnel = 0.02f;
    ...
    ///   The foam intensity
    /// </summary>
    [Range(0.0f, 3.0f)] public float foamIntensity = 1.0f;
    ...
    /// </summary>
    [HideInInspector] public Shader waveOverlaySdr;
    ...
    /// </summary>
    private readonly WaveQuery m_query = new(0.0f, 0.0f);
    ...
    /// </summary>
    public static float MAX_WAVE_HEIGHT => MAX_SPECTRUM_WAVE_HEIGHT + MAX_OVERLAY_WAVE_HEIGHT;
    ...
    /// </summary>
    public int CameraCount => m_cameraData.Count;
    ...
    private void Start()
    {
      try
      {
        ...
        for (var i = 0; i < 4; i++) T2S[1, i] = -T2S[1, i];
    ...
    private void Update()
    {
      try
      {
        ...
        var sr = Mathf.Lerp(2e-5f, 2e-2f, specularRoughness);
        ...
        Shader.SetGlobalTexture("Ceto_FoamTexture0",
          foamTexture0.tex != null ? foamTexture0.tex : Texture2D.whiteTexture);
        Shader.SetGlobalVector("Ceto_FoamTextureScale0", foamParam0);

        Shader.SetGlobalTexture("Ceto_FoamTexture1",
          foamTexture1.tex != null ? foamTexture1.tex : Texture2D.whiteTexture);
    ...
    private void OnDestroy()
    {
      try
      {
        ...
        var tmp = new List<Camera>(m_cameraData.Keys);
        foreach (var cam in tmp) RemoveCameraData(cam);
      }
      catch (Exception e)
      {
        LogError(e.ToString());
    ...
    public Vector3 CalculateWindDirVector()
    {
      ...
      var u = -Mathf.Cos(theta);
      var v = Mathf.Sin(theta);

      var dir = new Vector3(u, 0.0f, v);
    ...
    public void RemoveCameraData(Camera cam)
    {
      ...
      if (data.overlay != null) OverlayManager.DestroyBuffers(data.overlay);
    ...
    public Vector3 QueryNormal(float x, float z)
    {
      ...
      var hx0 = QueryWaves(x - d, z);
      var hz0 = QueryWaves(x, z - d);

      var hx1 = QueryWaves(x + d, z);
      var hz1 = QueryWaves(x, z + d);

      var dx = hx0 - hx1;
      var dz = hz0 - hz1;

      var n = new Vector3(dx, Mathf.Sqrt(1.0f - dx * dx - dz * dz), dz);
    ...
    public void RenderWaveOverlays(GameObject go)
    {
      try
      {
        ...
        {
          var normalSize = data.settings != null ? data.settings.normalOverlaySize : normalOverlaySize;
          var heightSize = data.settings != null ? data.settings.heightOverlaySize : heightOverlaySize;
          var foamSize = data.settings != null ? data.settings.foamOverlaySize : foamOverlaySize;
          var clipSize = data.settings != null ? data.settings.clipOverlaySize : clipOverlaySize;
