...
namespace Razomy.Unity.Scripts.Spectrum.Conditions
{
  public class UnifiedPhillipsSpectrumCondition : WaveSpectrumCondition
  {
      ...
      : base(size, numGrids)
    {
      ...
      Key = new UnifiedSpectrumConditionKey(windSpeed, waveAge, size, windDir, SPECTRUM_TYPE.UNIFIED_PHILLIPS,
        numGrids);
    ...
    {
      var key = Key as UnifiedSpectrumConditionKey;

      var uspectrum = new UnifiedSpectrum(key.WindSpeed, key.WindDir, key.WaveAge);

      var pspectrum = new PhillipsSpectrum(key.WindSpeed, key.WindDir);
