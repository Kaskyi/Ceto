...
namespace Ceto
{
  ...
  {
    private readonly DisplacementBufferCPU m_buffer;

    private readonly FourierCPU m_fourier;
    ...
    private void PerformSingleFourier()
    {
      ...
      if (read != WaveSpectrumBufferCPU.READ)
        throw new InvalidOperationException("Fourier transform did not result in the read buffer at index " +
                                            WaveSpectrumBufferCPU.READ);
    ...
    private void ProcessData(int index, Color[] result, Vector4[] data, int numGrids)
    {
      ...
      var size = m_buffer.Size;

      var displacements = m_buffer.GetWriteDisplacements();
