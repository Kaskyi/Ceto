...
namespace Razomy.Unity.Scripts.Spectrum
{
  ...
  public class WaveSpectrum : OceanComponent
  {
    ...
    /// </summary>
    [Range(1, 4)] public int numberOfGrids = 4;
    ...
    /// </summary>
    [Range(0.0f, MAX_CHOPPYNESS)] public float choppyness = 0.8f;

    /// <summary>
    ///   Controls the amount of foam.
    /// </summary>
    [Range(0.0f, MAX_FOAM_AMOUNT)] public float foamAmount = 1.0f;

    /// <summary>
    ///   Controls the foam coverage at the wave peaks.
    /// </summary>
    [Range(0.0f, MAX_FOAM_COVERAGE)] public float foamCoverage = 0.1f;

    /// <summary>
    ///   A higher wind speed creates bigger waves.
    /// </summary>
    [Range(0.0f, MAX_WIND_SPEED)] public float windSpeed = 8.0f;

    /// <summary>
    ///   Controls how long the waves last for.
    ///   A higher values means the waves decay faster and are shorter lived.
    /// </summary>
    [Range(MIN_WAVE_AGE, MAX_WAVE_AGE)] public float waveAge = 0.64f;

    /// <summary>
    ///   Scales the time value used to generate waves from.
    /// </summary>
    [Range(0.0f, MAX_WAVE_SPEED)] public float waveSpeed = 1.0f;
    ...
    /// </summary>
    [HideInInspector] public Shader initSlopeSdr, initDisplacementSdr, initJacobianSdr, fourierSdr;

    /// <summary>
    ///   Required shaders that should be bound to the script.
    /// </summary>
    [HideInInspector] public Shader slopeCopySdr, displacementCopySdr, foamCopySdr;

    /// <summary>
    ///   Used to read displacement data from the GPU.
    /// </summary>
    [HideInInspector] public ComputeShader readSdr;

    /// <summary>
    ///   All the scale parameters needed for wave query's.
    /// </summary>
    private readonly QueryGridScaling m_queryScaling = new();
    ...
    /*public*/
    private float foamSmoothing = 2.0f;
    ...
    /// </summary>
    private DictionaryQueue<WaveSpectrumConditionKey, WaveSpectrumCondition> m_conditionCache = new();
    ...
    /// </summary>
    private WaveSpectrumBuffer m_displacementBuffer, m_slopeBuffer, m_jacobianBuffer;
    ...
    /// </summary>
    private Material m_slopeCopyMat, m_displacementCopyMat, m_foamCopyMat;

    /// <summary>
    ///   Materials used to init the fourier data for the GPU buffers.
    /// </summary>
    private Material m_slopeInitMat, m_displacementInitMat, m_foamInitMat;
    ...
    /*public*/
    private float slopeSmoothing = 1.0f;

    /// <summary>
    ///   The textures holding the heights for each grid size.
    /// </summary>
    public IList<RenderTexture> DisplacementMaps => m_displacementMaps;

    /// <summary>
    ///   The textures holding the slope for each grid size.
    /// </summary>
    public IList<RenderTexture> SlopeMaps => m_slopeMaps;

    /// <summary>
    ///   The textures holding the foam for each grid size.
    /// </summary>
    public IList<RenderTexture> FoamMaps => m_foamMaps;
    ...
    /// </summary>
    public bool IsCreatingNewCondition => m_conditions == null ? false : m_conditions[1] != null;

    /// <summary>
    ///   Gets the grid sizes.
    /// </summary>
    public Vector4 GridSizes { get; private set; } = Vector4.one;

    /// <summary>
    ///   The choppyness as a vector, one for each grid.
    /// </summary>
    public Vector4 Choppyness { get; private set; } = Vector4.one;

    /// <summary>
    ///   Max number of conditions cached.
    /// </summary>
    public int MaxConditionCacheSize { get; set; } = 10;

    /// <summary>
    ///   Gets the displacement buffer.
    /// </summary>
    public IDisplacementBuffer DisplacementBuffer => m_displacementBuffer as IDisplacementBuffer;
    ...
    private void Update()
    {
      try
      {
        ...
        var time = m_ocean.OceanTime.Now * waveSpeed;
        ...
        var numGrids = m_conditions[0].Key.NumGrids;
    ...
    {
      Choppyness = m_conditions[0].Choppyness * Mathf.Clamp(choppyness, 0.0f, MAX_CHOPPYNESS);
      GridSizes = m_conditions[0].GridSizes;
    ...
    private void ReadFromGPU(int numGrids)
    {
      ...
      if (!disableReadBack && readSdr != null && m_readBuffer != null && supportDX11)
      {
        ...
        var buffer = DisplacementBuffer.GetReadDisplacements();
    ...
    private void GenerateFoam(float time)
    {
      ...
      var sqrMag = foamChoppyness.sqrMagnitude;
    ...
    {
      var size = m_bufferSettings.size;
      var aniso = 9;
      ...
      for (var i = 0; i < m_displacementMaps.Length; i++)
        CreateMap(ref m_displacementMaps[i], "Displacement", format, size, aniso);

      for (var i = 0; i < m_slopeMaps.Length; i++) CreateMap(ref m_slopeMaps[i], "Slope", format, size, aniso);

      for (var i = 0; i < m_foamMaps.Length; i++) CreateMap(ref m_foamMaps[i], "Foam", format, size, aniso);
    }
    ...
    /// </summary>
    private WaveSpectrumConditionKey NewSpectrumConditionKey(int fourierSize, float windSpeed, float windDir,
      float waveAge)
    {
