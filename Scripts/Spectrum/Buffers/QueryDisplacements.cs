...
namespace Ceto
{
  public static class QueryDisplacements
  {
    ...
    public static void QueryWaves(WaveQuery query, int enabled, IList<InterpolatedArray2f> displacements,
      QueryGridScaling scaling)
    {
      if (displacements.Count != GRIDS)
        throw new InvalidOperationException("Query Displacements requires a displacement buffer for each of the " +
                                            GRIDS + " grids.");

      if (displacements[0].Channels != CHANNELS)
        throw new InvalidOperationException("Query Displacements requires displacement buffers have " + CHANNELS +
                                            " channels.");

      //Only these modes are relevant to this code.
      if (query.mode != QUERY_MODE.DISPLACEMENT && query.mode != QUERY_MODE.POSITION) return;

      var x = query.posX + scaling.offset.x;
      var z = query.posZ + scaling.offset.z;
      ...
      if (enabled == 1 || query.mode == QUERY_MODE.DISPLACEMENT)
      {
        ...
        query.result.height = query.result.displacement[0].y + query.result.displacement[1].y +
                              query.result.displacement[2].y + query.result.displacement[3].y;
      ...
      else
      {
        ...
        do
        {
          ...
          displacementX = query.result.displacement[0].x + query.result.displacement[1].x +
                          query.result.displacement[2].x + query.result.displacement[3].x;
          displacementZ = query.result.displacement[0].z + query.result.displacement[1].z +
                          query.result.displacement[2].z + query.result.displacement[3].z;
    ...
    private static void SampleHeightOnly(Vector3[] d, IList<InterpolatedArray2f> displacements, bool[] sample, float x,
      float z, QueryGridScaling scaling)
    {
      float u, v;
      //The array needed to sample the results is kept in the scaling
      //class to reduce memory allocations.
      var result = scaling.result;
    ...
    private static void Sample(Vector3[] d, IList<InterpolatedArray2f> displacements, bool[] sample, float x, float z,
      QueryGridScaling scaling)
    {
      float u, v;
      //The array needed to sample the results is kept in the scaling
      ...
      ICancelToken token)
    {
      ...
      var size = displacements[0].SX;

      var ninf = new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity);
      var pinf = new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity);

      Vector3[] max = { ninf, ninf, ninf, ninf };
      Vector3[] min = { pinf, pinf, pinf, pinf };

      var h = new float[CHANNELS];
