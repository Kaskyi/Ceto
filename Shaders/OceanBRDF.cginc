...
half3 ReflectedSunRadianceNice(half3 V, half3 N, half3 L, fixed fresnel)
{
    ...
    half p = exp(-0.5 * (zetax * zetax / Ceto_SpecularRoughness + zetay * zetay / Ceto_SpecularRoughness)) / (2.0 * M_PI
        * Ceto_SpecularRoughness);
    ...
    return (L.y < 0)
               ? 0.0
               : Ceto_SpecularIntensity * p / ((1.0 + Lambda(zL, sigmaL2) + Lambda(zV, sigmaV2)) * zV * zH2 * zH2 *
                   4.0);
}
...
half ReflectedSunRadianceFast(half3 V, half3 N, half3 L, fixed fresnel)
{
    ...
    half hn = dot(H, N);
    half p = exp(-2.0 * ((1.0 - hn * hn) / Ceto_SpecularRoughness) / (1.0 + hn)) / (4.0 * M_PI *
        Ceto_SpecularRoughness);
