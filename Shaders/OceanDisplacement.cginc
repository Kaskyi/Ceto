...
float4 OceanPos(float4 uv)
{
    ...
    float4 p = lerp(lerp(Ceto_Interpolation[0], Ceto_Interpolation[1], uv.x),
                    lerp(Ceto_Interpolation[3], Ceto_Interpolation[2], uv.x), uv.y);
    p = p / p.w;

    //Find the world position of the screens center position.
    float4 c = lerp(lerp(Ceto_Interpolation[0], Ceto_Interpolation[1], 0.5),
                    lerp(Ceto_Interpolation[3], Ceto_Interpolation[2], 0.5), 0.5);
    c = c / c.w;
...
*/
void OceanNormalAndFoam(float4 uv, float4 st, float3 worldPos, out half3 norm1, out half3 norm2, out half3 norm3,
                        out half3 unmaskedNorm2, out fixed4 foam)
{
    float4 oceanPos = OceanPos(uv);
